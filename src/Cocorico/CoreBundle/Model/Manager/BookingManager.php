...
class BookingManager extends BaseManager
{
    ...
    public function __construct(
        ...
        $this->feeGeneral = $parameters['cocorico_fee_general'];
     ...
     * @param TimeRange|bool $timeRange
     *
     * @return Booking
     */
...
//                } else {
//                    $booking->setAmountFeeAsAsker($this->feeGeneral);
//                }
                // we need only general fee
                $booking->setAmountFeeAsAsker($this->feeGeneral);
            }
            ...
            if ($offerer) {
                ...
                if (($this->feeGeneral === 0) && ($feeAsOfferer || $feeAsOfferer === 0)) {
                    $booking->setAmountFeeAsOfferer(($feeAsOfferer / 100) * $booking->getAmount());
                } else {
                    $booking->setAmountFeeAsOfferer($this->feeGeneral);
                }
            }
            ...
            $booking->setAmountTotal($booking->getAmount());
        }

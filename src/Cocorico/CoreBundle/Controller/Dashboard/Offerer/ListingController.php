...
class ListingController extends Controller
{
    ...
    /**
     * This function is calculating the fees need to be deducted for offer
     * it may be fixed if no % deducting.
     *
     * @param User $user
     *
     * @return float|mixed
    ...
    {
        $generalFee = $this->container->getParameter('cocorico.fee_general');
        $feeAsOfferer = $this->container->getParameter('cocorico.fee_as_offerer');
        if (($generalFee === 0) && ($user->getFeeAsOfferer() || $user->getFeeAsOfferer() === 0)) {
            $feeAsOfferer = $user->getFeeAsOfferer() / 100;
        } else {
            $feeAsOfferer = $generalFee;
        }
